...
def get_mixture_box_length(config: WorkflowInput, mixture: MixtureInput) -> float:
    return mixture.box_length_a or config.packmol.box_length
...
def prepare_structure_input(config: WorkflowInput) -> WorkflowInput:
    ...
    for mixture in config.structure.mixtures:
        if not np.isclose(sum(c.volume_fraction for c in mixture.components), 1.0):
            raise ValueError(f"Volume fractions for {mixture.name} must sum to 1.0.")
        for component in mixture.components:
            solvent_path = _path(config.workdir, component.structure_file)
            ...
            component.molar_mass_g_mol = molecular_mass_g_mol(solvent_path)
            if component.n_molecules is None:
                component.n_molecules = estimate_molecule_count(
                    get_mixture_box_length(config, mixture),
                    component.density_g_cm3 * component.volume_fraction,
                    component.molar_mass_g_mol,
                )
    return config
...
def estimate_system_atom_count(config: WorkflowInput, mixture: MixtureInput) -> int:
    ...
    component_atoms = 0
    for component in mixture.components:
        if component.n_atoms is None or component.n_molecules is None:
            raise ValueError(f"Solvent metadata is incomplete for {component.name}.")
        component_atoms += component.n_atoms * component.n_molecules
    return int(config.structure.n_opa_atoms + component_atoms)


def estimate_pfp_neighbor_count(config: WorkflowInput, mixture: MixtureInput) -> int:
    n_atoms = estimate_system_atom_count(config, mixture)
    volume_a3 = get_mixture_box_length(config, mixture) ** 3
...
def validate_md_system_sizes(
    ...
    too_large = []
    for mixture in config.structure.mixtures:
        if selected is not None and _safe_name(mixture.name).lower() not in selected:
            continue
        n_atoms = estimate_system_atom_count(config, mixture)
        if n_atoms > max_atoms:
            counts = ", ".join(f"{c.name}={c.n_molecules}" for c in mixture.components)
            too_large.append((mixture.name, n_atoms, counts))

    if too_large:
        lines = [
            f"{name}: {n_atoms} atoms ({counts})"
            for name, n_atoms, counts in too_large
        ]
        raise ValueError(
...
def validate_mlip_neighbor_limit(
    ...
    for mixture in config.structure.mixtures:
        if selected is not None and _safe_name(mixture.name).lower() not in selected:
            continue
        n_atoms = estimate_system_atom_count(config, mixture)
        neighbors = estimate_pfp_neighbor_count(config, mixture)
        if neighbors > max_neighbors:
            counts = ", ".join(f"{c.name}={c.n_molecules}" for c in mixture.components)
            too_large.append((mixture.name, n_atoms, counts, neighbors))

    if too_large:
        lines = [
            (
                f"{name}: estimated_neighbors={neighbors:,}, atoms={n_atoms:,}, "
                f"solvent_molecules={counts}"
            )
            for name, n_atoms, counts, neighbors in too_large
        ]
        raise ValueError(
...
def write_packmol_input_for_mixture(config: WorkflowInput, mixture: MixtureInput) -> Path:
    p = config.packmol
    box_length = get_mixture_box_length(config, mixture)
    ...
    opa_xyz = structure_to_packmol_xyz(
        ...
        label=f"{_safe_name(mixture.name)}_fixed" if p.fixed_opa else None,
    )
    output_xyz = _packmol_path(config, mixture, p.output_xyz_template)
...
""".strip()
    ...
    component_blocks = []
    for component in mixture.components:
        solvent_xyz = structure_to_packmol_xyz(config, component.structure_file)
...
""".strip()
    ...
    path = _packmol_path(config, mixture, p.packmol_input_template)
...
def write_packmol_inputs(config: WorkflowInput) -> list[Path]:
    prepare_structure_input(config)
    return [
        write_packmol_input_for_mixture(config, mixture)
        for mixture in config.structure.mixtures
    ]
...
def run_packmol(
    ...
    *,
    ...
    outputs = []
    for mixture in config.structure.mixtures:
        inp = write_packmol_input_for_mixture(config, mixture)
        ...
        print(f"Packmol finished for {mixture.name}: {output_xyz}")
        outputs.append(output_xyz)
    return outputs
...
def configure_system_outputs(
    config: WorkflowInput,
    mixture: MixtureInput,
    results_dir: str | Path = "results",
) -> WorkflowInput:
    """Return a config copy with per-system input and output paths."""
    c = deepcopy(config)
    c.packmol.box_length = get_mixture_box_length(config, mixture)
    name = _safe_name(mixture.name)
    system_dir = _path(c.workdir, results_dir) / name
    system_dir.mkdir(parents=True, exist_ok=True)

    c.md.input_xyz = str(_packmol_path(c, mixture, c.packmol.output_xyz_template))
    ...
    *,
    ...
    for mixture in config.structure.mixtures:
        ...
        if skip_existing and traj_path.exists() and final_xyz_path.exists():
            print(f"Skipping existing result: {mixture.name}")
            atoms = None
        else:
            ...
            summary_rows.append(
                {
                    ...
                    "component_counts": "; ".join(
                        f"{c.name}={c.n_molecules}" for c in mixture.components
                    ),
                    "target_volume_fractions": "; ".join(
                        f"{c.name}={c.volume_fraction:g}" for c in mixture.components
                    ),
...
def compare_mixture_opa_motion(
    config: WorkflowInput,
    outputs: dict[str, dict[str, Any]],
    results_dir: str | Path = "results",
) -> None:
    """Save overlaid OPA displacement/force plots and one long-format CSV."""
    import matplotlib.pyplot as plt
    import pandas as pd

    frames = []
    for name, result in outputs.items():
        df = result.get("analysis")
        if df is None:
            continue
        comparison_df = df.copy()
        comparison_df.insert(0, "system", name)
    ...
    comparison.to_csv(results_root / "opa_mixture_comparison.csv", index=False)

    plt.figure()
    for name, df in comparison.groupby("system", sort=False):
        plt.plot(df["time_ps"], df["OPA_disp_norm_A"], label=name)
    plt.xlabel("Time / ps")
    plt.ylabel("OPA COM displacement / A")
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_root / "opa_displacement_comparison.png", dpi=300)
    plt.close()

    plt.figure()
    for name, df in comparison.groupby("system", sort=False):
        plt.plot(df["time_ps"], df["F_OPA_norm_eV_A"], label=name)
    plt.xlabel("Time / ps")
    plt.ylabel("|F_OPA| / eV A$^{-1}$")
    plt.legend()
    plt.tight_layout()
    plt.savefig(results_root / "opa_force_comparison.png", dpi=300)
    plt.close()
...
def build_config() -> WorkflowInput:
    ...
    count_overrides = {
        _safe_name(name).lower(): counts
        for name, counts in SYSTEM_MOLECULE_COUNTS.items()
    }
    for mixture in config.structure.mixtures:
        key = _safe_name(mixture.name).lower()
        mixture.box_length_a = overrides.get(key, mixture.box_length_a)
        counts = count_overrides.get(key, {})
        for component in mixture.components:
            if component.name in counts:
                component.n_molecules = counts[component.name]
    return config
...
def main() -> None:
    ...
    for mixture in config.structure.mixtures:
        box_length = get_mixture_box_length(config, mixture)
        n_atoms_total = estimate_system_atom_count(config, mixture)
        component_text = ", ".join(
            f"{c.name}: N={c.n_molecules}, target_v={100*c.volume_fraction:g}%"
            for c in mixture.components
        )
        print(
            f"{mixture.name:24s}  box={box_length:g} A  {component_text}  "
            f"atoms={n_atoms_total}  "
            f"estimated_neighbors={estimate_pfp_neighbor_count(config, mixture):,}"
        )
    ...
    if RUN_MD:
        ...
        if RUN_ANALYSIS:
            compare_mixture_opa_motion(config, all_outputs, RESULTS_DIR)
